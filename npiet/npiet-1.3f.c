...
usage (int rc)
{
  ...
  fprintf (stderr, "\t-w         - show warnings (default: off)\n");
...
/* show warnings - like div by zero or so: */
int warn = 0;
...
piet_action (int c_col, int a_col, int num_cells, char *msg)
{
  ...
  case 2:
	...
	wprintf ("\nwarning: divide failed: division by zero\n"); wflush ();
	...
	wprintf ("\nwarning: mod failed: stack underflow \n"); wflush ();
      } else if (stack [num_stack - 1] == 0) {
 	/* try to put a undefined, but visible value on stack: */
	stack [num_stack - 2] = 99999999;
	num_stack--;
	tprintf ("info: mod failed: modulo by zero\n");
	wprintf ("\nwarning: mod failed modulo by zero\n"); wflush ();
      } else {
	stack [num_stack - 2] = stack [num_stack - 2] % stack [num_stack - 1];
	num_stack--;
      }
